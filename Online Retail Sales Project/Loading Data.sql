...
# Create Additional Table to Calculate CLV
CREATE TABLE Sum_Of_Customer_Lifespans AS
SELECT CustomerID, MAX(InvoiceDate) as latest_purchase, MIN(InvoiceDate) as first_purchase, DATEDIFF(MAX(InvoiceDate), MIN(InvoiceDate)) as diff
FROM transactions
GROUP BY CustomerID;